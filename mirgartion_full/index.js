// ============================================================
// Full Access to PostgreSQL Migration Tool
// Phase 1: Access -> Temporary PostgreSQL (streaming)
// Phase 2: Temp PostgreSQL -> Production PostgreSQL (Prisma)
// ============================================================
...
// Configuration
// ============================================================
const ACCESS_PATH = path.resolve(process.env.ACCESS_DB_PATH || "./DatabaseBig.accdb");
...
const CONN_STR = `Provider=${OLEDB_PROVIDER};Data Source=${ACCESS_PATH};`;

// Temporary PostgreSQL configuration (port 5433)
const TEMP_PG_CONFIG = {
    host: process.env.PG_HOST || "localhost",
    port: parseInt(process.env.PG_PORT || "5433", 10),
    database: process.env.PG_DATABASE || "access_migration",
    user: process.env.PG_USER || "postgres",
    password: process.env.PG_PASSWORD || "postgres",
};

// PowerShell helper scripts
const SCHEMA_SCRIPT = path.join(__dirname, "_ps_schema.ps1");
const STREAM_SCRIPT = path.join(__dirname, "_ps_stream.ps1");
    ...
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            if (i < maxRetries - 1) {
                ...
                throw new Error("Cannot connect to Temporary PostgreSQL: " + err.message);
            }
        }
    }
    console.log("");
...
async function streamAndInsertFromPowerShell(pg, table) {
    return new Promise((resolve, reject) => {
        const ps = spawn("powershell.exe", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", STREAM_SCRIPT, "-ConnStr", CONN_STR, "-TableName", table.name], { stdio: ["ignore", "pipe", "pipe"] });
        ps.stdout.setEncoding("utf8");
        ps.stderr.setEncoding("utf8");
...
async function migrateAccessToTempDb() {
    console.log("\n[Phase 1] Migrating Access to Temporary PostgreSQL...\n");

    if (!fs.existsSync(ACCESS_PATH)) {
        throw new Error(`Access file not found: ${ACCESS_PATH}`);
    }
    ...
    for (let i = 0; i < tables.length; i++) {
        ...
        try {
            await streamAndInsertFromPowerShell(pg, table);
        } catch (err) {
...
async function migrateTempToProduction() {
    ...
    const { id } = await createDefaultAdminsAndServices();
...
async function main() {
    ...
    console.log("============================================\n");
    console.log(`Access DB : ${ACCESS_PATH}`);
    console.log(`Temp PG   : ${TEMP_PG_CONFIG.host}:${TEMP_PG_CONFIG.port}/${TEMP_PG_CONFIG.database}`);
    console.log(`Prod PG   : ${process.env.DATABASE_URL}`);
    console.log(`Batch Size: ${BATCH_SIZE}\n`);

    try {
        // Phase 1: Access -> Temporary PostgreSQL
        await migrateAccessToTempDb();
        ...
        console.log("\nTo stop the temporary database: docker-compose down");
    } catch (err) {
