...
DATABASE_URL="postgresql://postgres:postgres@localhost:5432/mbdatabase"
...
- **Microsoft Access Driver**: Install `Microsoft.ACE.OLEDB.12.0` (Microsoft Access Database Engine)
...
**Authentication**: Requires `superAdmin` role (send `x-access-token` header)

**Request**: `multipart/form-data` with `file` field containing `.accdb` or `.mdb` file

### How Migration Works

1. **Phase 1**: Streams Access data → `access_migration` database (temp)
2. **Phase 2**: Transforms and migrates → `mbdatabase` (production)

### Example (using curl)

```bash
curl -X POST http://localhost:8080/api/v1/migration/upload \
  -H "x-access-token: YOUR_TOKEN" \
  -F "file=@your_database.accdb"
```
