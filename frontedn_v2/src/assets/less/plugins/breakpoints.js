...
function lookupVariable(context, variableName) {
	...
	return less.tree.Variable.prototype.find(frames, frame => {
...
function getBreakpoints(context, breakpoints) {
	...
	for (const key in rulesetMap) {
		...
		rulesetMap[key] = new less.tree.Dimension(number, unit);
	}
...
less.functions.add("breakpoint-next", function ({ value: breakpointName }, breakpoints) {
	...
	if (breakpointIndex === -1)
		return new less.tree.Quoted("\"");

	// Next breakpoint is null for the last breakpoint.
	if ((breakpointIndex + 1) === breakpointNames.length)
		return new less.tree.Quoted("\"");

	return new less.tree.Quoted("\"", breakpointNames[breakpointIndex + 1]);
});

less.functions.add("breakpoint-min", function ({ value: breakpointName }, breakpoints) {
	const breakpointsMap  = getBreakpoints(this.context, breakpoints);
	const breakpointNames = Object.keys(breakpointsMap);
	const breakpointIndex = breakpointNames.indexOf(breakpointName);
...
less.functions.add("breakpoint-max", function ({ value: breakpointName }, breakpoints) {
	const breakpointsMap  = getBreakpoints(this.context, breakpoints);
	const breakpointNames = Object.keys(breakpointsMap);
	const breakpointIndex = breakpointNames.indexOf(breakpointName);
	...
	return new less.tree.Dimension(nextBreakpoint.value - 0.02, nextBreakpoint.unit);
});

less.functions.add("breakpoint-infix", function ({ value: breakpointName }, breakpoints) {
	const breakpointsMap  = getBreakpoints(this.context, breakpoints);
	const breakpointNames = Object.keys(breakpointsMap);
	const breakpointIndex = breakpointNames.indexOf(breakpointName);
	...
	return new less.tree.Quoted("\"", `-${breakpointName}`);
});
