...
const Routes = () => {
	...
	const [permissionProtectedRouters, setPermissionProtectedRoutes] = React.useState([]);

	useEffect(() => {
		if(role) {
			setPermissionProtectedRoutes(protectedRoutes.filter(route => route.role.includes(role)));
		}
	}, [role]);
