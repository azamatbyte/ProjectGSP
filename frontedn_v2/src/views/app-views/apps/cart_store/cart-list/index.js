...
const CartList = () => {
  ...
  const [isModalVisibleSpecialAnalysis, setIsModalVisibleSpecialAnalysis] = useState(false);
  ...
  const onDragEnd = async ({ active, over }) => {
    if (active.id !== over?.id) {
      ...
      try {
        ...
        await SessionService.swapSessions(SESSION_TYPES.SESSION, activeIndex, overIndex);
  ...
  const editRegistration = (row) => {
    if (row?.model === "registration" || row?.model === "registration4") {
      navigate(
  ...
  const addRelative = (row) => {
    if (row?.model === "registration") {
      navigate(
        `/app/apps/relative/add-relative/${row.registrationId}?redirect=/app/cart-list`
      );
    }
  };
  ...
  const clearHandle = async () => {
    try {
      ...
      const response = await SessionService.clear(SESSION_TYPES.SESSION);
  ...
  const exportExcel = async () => {
    try {
      const response = await RegistrationFourService.exportData({
        type: SESSION_TYPES.SESSION,
      });
      if (response?.status === 200) {
        const a = document.createElement("a");
        a.href = (
          response?.data?.link +
          "?newFileName=Excel_report"
        ).trim();
        a.setAttribute("download", "Excel_report.xlsx");
  ...
  const handleExportRelativesSP = async (type, signListIds) => {
    ...
    try {
      ...
      setSelectedRowKeys([]);
      setSelectedValues([]);
      setIsModalVisibleSpecialAnalysis(false);
    }
  };
  ...
  const generateReport = async (name) => {
    try {
      ...
      if (response?.status === 200) {
        ...
        a.href = (
          response?.data?.link +
          "?newFileName=Заключение"
        ).trim();
        a.setAttribute("download", "Заключение.xlsx");
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
      }
  ...
  return (
    <Card style={{ width: "100%" }}>
      ...
      <Modal
        title={t("export_special_analysis")}
        open={isModalVisibleSpecialAnalysis}
        onOk={() => handleExportRelativesSP(SESSION_TYPES.SESSION, selectedValues)}
        onCancel={() => setIsModalVisibleSpecialAnalysis(false)}
      >
        <Typography.Text
          style={{ width: "100%", cursor: "pointer", marginBottom: "10px" }}
        >
          {t("export_special_analysis")}
        </Typography.Text>
        <Select
          mode="multiple" // enable multiple selections
          showSearch
          ...
          <div className="d-flex flex-column align-items-end w-100">
            ...
            >
              <Col xs={24} sm={12} md={12} lg={10} xl={8} xxl={5}>
                <div className="ml-20 mb-3">
                  {/* <Button
                    onClick={() => generateReport("Заключение")}
                    type="primary"
            ...
            <Row gutter={24} className="w-100 mb-2 justify-content-end">
              ...
              <Col xs={24} sm={13} md={13} lg={13} xl={10} xxl={5}>
                <Button
                  className="w-100 mb-2"
                  onClick={() => setIsModalVisibleSpecialAnalysis(true)}
                  type="primary"
                >
                  <FileExcelOutlined />
