...
const LogList = ({id}) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				setLoading(true);
				const response = await FormService.getLogList("WorkPlace",id,field,pageNumber,pageSize);
		...
		fetchData();
	}, [field,pageNumber,pageSize, t, id]);
	...
	// 	navigate(`/app/apps/register/register-log-info/${row.id}`);
	// };
	...
	const tableColumns = [
		...
		},
		{
			...
			render: (_,elm) => (
				<div>
					{elm?.executor?.first_name? elm?.executor?.first_name : ""} {elm?.executor?.last_name? elm?.executor?.last_name : ""}
				</div>
			),
