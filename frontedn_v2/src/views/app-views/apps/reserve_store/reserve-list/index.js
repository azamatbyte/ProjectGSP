...
const CartList = () => {
  ...
  const onDragEnd = async ({ active, over }) => {
    if (active.id !== over?.id) {
      ...
      try {
        ...
        await SessionService.swapSessions(SESSION_TYPES.RESERVE, activeIndex, overIndex);
  ...
  const clearHandle = async () => {
    try {
      ...
      const response = await SessionService.clear(SESSION_TYPES.RESERVE);
  ...
  return (
    <Card style={{ width: "100%" }}>
      ...
      <Modal
        ...
        onOk={() => handleExportRelativesSP(SESSION_TYPES.RESERVE, selectedValues)}
