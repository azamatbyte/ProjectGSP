...
const fetchExecutors = async (searchText) => {
  try {
    const response = await AuthService.getList(1, 15, searchText, "active");
...
const StatisticsByYear = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setExecutorOptions(
        [{value: "all", label: t("all")}, ...executors.map((item) => ({
          value: item.id,
          label: `${item.last_name} ${item.first_name}${item.father_name ? " " + item.father_name : ""}`.trim(),
        }))]
  ...
  const debouncedFetchExecutors = debounce(async (searchText) => {
    if (searchText.length > 1) {
      ...
      setExecutorOptions(
        [{value: "all", label: t("all")}, ...data.map((item) => ({
          value: item.id,
          label: `${item.last_name} ${item.first_name}${item.father_name ? " " + item.father_name : ""}`.trim(),
        }))]        
      );
  ...
  const downloadReport = async () => {
    try {
      const response = await StatisticsService.reportStatementsByYear({
        years: search?.years,
        forms: search?.forms,
      ...
      if (response?.data?.code === 200) {
        ...
        const filename = "Статистика по годам";
